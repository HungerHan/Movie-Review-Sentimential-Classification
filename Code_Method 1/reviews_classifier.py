...
def cross_validation_model_analysis(all_features, all_labels):
	...
	# OrderedDict 会根据放入元素的先后顺序进行排序。所以输出的值是排好序的
	...
	for transformer_name, transformer in transformers_dict.items():  # items() 函数以列表返回可遍历的(键, 值) 元组数组。
		...
		for estimator_name, estimator in estimators_dict.items():
			...
			model = Pipeline(steps)						#可以把多个“处理数据的节点”按顺序打包在一起，数据在前一个节点处理之后的结果，转到下一个节点处理
...
####################################################
			accuracy = round(accuracy_score(all_labels, predicted_labels))
